...
dados_ba = fetch_datasus(year_start = 2015,
                      year_end = 2024,
                      uf = "MS",
...
Save dados_ba only a single time

```{r, eval=F}
saveRDS(dados_ba, "./data/dados_ba.rds")
```
...
```{r}
dados_ba <- readRDS("./data/dados_ba.rds")
```
\
...
```{r sort dataframe}
racacor_df <- dados_ba %>% count(RACACOR, sort = TRUE)
```
...
```{r}
library(lubridate)
dados_selected <- dados_ba %>%
  select(CONTADOR, RACACOR, DTOBITO, CAUSABAS, DTNASC, IDADE) %>%
  mutate(YMD = dmy(dados_ba$DTOBITO))
...
```{r}
library('readxl')
Disparity_coeficient_BA <- read_excel("./data/Disparity_coeficient_Sum_PardosBlacks_BA.xlsx")
```


```{r}
ggplot(Disparity_coeficient_BA, aes(x=year, y=value, group=racacor, color=racacor)) +
