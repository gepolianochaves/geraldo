...
dados_pe = fetch_datasus(year_start = 2015,
                      year_end = 2024,
                      uf = "PE",
...
Save dados_pe only a single time

```{r, eval=F}
saveRDS(dados_pe, "./data/dados_pe.rds")
```
...
```{r}
dados_pe <- readRDS("./data/dados_pe.rds")
```
\
...
```{r sort dataframe}
racacor_df <- dados_pe %>% count(RACACOR, sort = TRUE)
```
...
```{r}
library(lubridate)
dados_selected <- dados_pe %>%
  select(CONTADOR, RACACOR, DTOBITO, CAUSABAS, DTNASC, IDADE) %>%
  mutate(YMD = dmy(dados_pe$DTOBITO))
