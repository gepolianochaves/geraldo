...
dados_norte = fetch_datasus(year_start = 2015,
                      year_end = 2024,
...
Save dados_norte only a single time

```{r, eval=F}
saveRDS(dados_norte, "./data/dados_norte.rds")
```
...
```{r}
dados_norte <- readRDS("./data/dados_norte.rds")
```
\
...
library(expss)
cross_cases(dados_norte, RACACOR)
```
...
```{r sort dataframe}
racacor_df <- dados_norte %>% count(RACACOR, sort = TRUE)
```
...
```{r}
library(lubridate)
dados_selected <- dados_norte %>%
  select(CONTADOR, RACACOR, DTOBITO, CAUSABAS, DTNASC, IDADE) %>%
  mutate(YMD = dmy(dados_norte$DTOBITO))
