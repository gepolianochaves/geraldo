...
Specify sheet with a number or name for the generation plot

```{r}
brGeneal <- read_excel("../geraldo/data/brGeneal.xlsx", sheet = "Sheet4")
```

```{r}
plotAncDes("Garcia D'Ávila", brGeneal, mAnc = 3, mDes = 6, vCol = "blue")
```

\

Specify sheet with a number or name for the generation plot (sheet = "Sheet5")

```{r}
brGeneal <- read_excel("../geraldo/data/brGeneal.xlsx", sheet = "Sheet5")
```
...
```{r}
plotAncDes("Garcia D'Ávila", brGeneal, mAnc = 4, mDes = 6, vCol = "blue")
```

Specify sheet with a number or name for the generation plot (sheet = "Sheet6")

```{r}
brGeneal <- read_excel("../geraldo/data/brGeneal.xlsx", sheet = "Sheet6")
plotAncDes("Garcia D'Ávila", brGeneal, mAnc = 8, mDes = 9, vCol = "blue")
```

\

Diniz I and D. Beatriz de Castela

\


```{r}
brGeneal <- read_excel("../geraldo/data/brGeneal.xlsx", sheet = "Sheet7")
plotAncDes("D. Beatriz de Castela", brGeneal, mAnc = 8, mDes = 9, vCol = "blue")
```


\

Starting with D. Afonso Henriques
...
plotAncDes("Afonso I de Portugal", brGeneal, mAnc = 3, mDes = 7, vCol = "blue")
```

\

Starting with Sancho I de Portugal
...
```{r}
brGeneal <- read_excel("../geraldo/data/brGeneal.xlsx", sheet = "Sheet8")
plotAncDes("Sancho I de Portugal", brGeneal, mAnc = 3, mDes = 7, vCol = "blue")
```
