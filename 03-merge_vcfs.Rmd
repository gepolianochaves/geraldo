...
## 3. Extract ped and map from merged vcf file for plink downstream analysis

\

Move merged file to another folder
...
Finally output to the mt_extracted.txt file:
...
for loop in the fasta folder maintaining fasta ID only
...
Create file with sample names matching GenBank IDs.
...
Read files into R and rename the columns
...
Read appended file into R and rename  columns

```{r sample IDs metadataIDs.txt}
...
cd ./data
## chr, pos, ref>alt
awk -F "<*>" '{print $1}' mt_extracted.txt | awk -F "," '{print $1}' | awk -F "\t" '{print $1"\t"$2"\t"$3">"$4}' >  file1.txt
awk -F "<*>" '{print $2}' mt_extracted.txt |  awk -F "," '{print $1}' > file2.txt
paste file2.txt file1.txt > file3.txt
```
...
```{r read genotypes extracted file}
...
```{r read genotypes appended file}
...
```{r select columns for function dcast()}
...
```{r Merge sampleIDs and mt_dcast }
...
```{r Merge sampleIDs and mt_dcast_appended }
...
```{r save mt_dcast_appended_binary}
