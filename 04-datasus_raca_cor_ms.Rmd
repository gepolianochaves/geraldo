...
```{r sort dataframe}
racacor_df <- dados_ms %>% count(RACACOR, sort = TRUE)
```
...
```{r}
library('readxl')
Disparity_coeficient_MS <- read_excel("./data/Disparity_coeficient_Sum_PardosBlacks_MS.xlsx")
```


```{r}
ggplot(Disparity_coeficient_MS, aes(x=year, y=value, group=racacor, color=racacor)) +
