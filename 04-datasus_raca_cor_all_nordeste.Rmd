...
dados_nordeste = fetch_datasus(year_start = 2015,
                      year_end = 2024,
                      uf = c("AM", "PA", "AP", "RR", "RO", "AC", "TO"),
                      information_system = "SIM-DO")
```

\

Save dados_nordeste only a single time

```{r, eval=F}
saveRDS(dados_nordeste, "./data/dados_nordeste.rds")
```
...
```{r}
dados_nordeste <- readRDS("./data/dados_nordeste.rds")
```
\
...
library(expss)
cross_cases(dados_nordeste, RACACOR)
```
...
```{r sort dataframe}
racacor_df <- dados_nordeste %>% count(RACACOR, sort = TRUE)
```
...
```{r}
library(lubridate)
dados_selected <- dados_nordeste %>%
  select(CONTADOR, RACACOR, DTOBITO, CAUSABAS, DTNASC, IDADE) %>%
  mutate(YMD = dmy(dados_nordeste$DTOBITO))
