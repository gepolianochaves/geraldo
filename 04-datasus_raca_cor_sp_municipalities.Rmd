...
dados_sp = fetch_datasus(year_start = 2015,
                      year_end = 2024,
                      uf = "SP",
...
Save dados_sp only a single time

```{r, eval=F}
saveRDS(dados_sp, "./data/dados_sp.rds")
```
...
```{r}
dados_sp <- readRDS("./data/dados_sp.rds")
```
\
...
```{r sort dataframe}
racacor_df <- dados_sp %>% count(RACACOR, sort = TRUE)
```
...
```{r}
library(lubridate)
dados_selected <- dados_sp %>%
  select(CONTADOR, RACACOR, DTOBITO, CAUSABAS, DTNASC, IDADE) %>%
  mutate(YMD = dmy(dados_sp$DTOBITO))
...
```{r}
library('readxl')
Disparity_coeficient_SP <- read_excel("./data/Disparity_coeficient_Sum_PardosBlacks_SP.xlsx")
```


```{r}
ggplot(Disparity_coeficient_SP, aes(x=year, y=value, group=racacor, color=racacor)) +
