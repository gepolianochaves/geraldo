...
dados_sul = fetch_datasus(year_start = 2015,
                      year_end = 2024,
                      uf = c("RS", "PR", "SC"),
...
Save dados_sul only a single time

```{r, eval=F}
saveRDS(dados_sul, "./data/dados_sul.rds")
```
...
```{r}
dados_sul <- readRDS("./data/dados_sul.rds")
```
\
...
```{r sort dataframe}
racacor_df <- dados_sul %>% count(RACACOR, sort = TRUE)
```
...
```{r}
library(lubridate)
dados_selected <- dados_sul %>%
  select(CONTADOR, RACACOR, DTOBITO, CAUSABAS, DTNASC, IDADE) %>%
  mutate(YMD = dmy(dados_sul$DTOBITO))
