...
dados_rs = fetch_datasus(year_start = 2015,
                      year_end = 2024,
                      uf = "RS",
...
Save dados_rs only a single time

```{r, eval=F}
saveRDS(dados_rs, "./data/dados_rs.rds")
```
...
```{r}
dados_rs <- readRDS("./data/dados_rs.rds")
```
\
...
```{r sort dataframe}
racacor_df <- dados_rs %>% count(RACACOR, sort = TRUE)
```
...
```{r}
library(lubridate)
dados_selected <- dados_rs %>%
  select(CONTADOR, RACACOR, DTOBITO, CAUSABAS, DTNASC, IDADE) %>%
  mutate(YMD = dmy(dados_rs$DTOBITO))
...
```{r}
library('readxl')
Disparity_coeficient_RS <- read_excel("./data/Disparity_coeficient_Sum_PardosBlacks_RS.xlsx")
```


```{r}
ggplot(Disparity_coeficient_RS, aes(x=year, y=value, group=racacor, color=racacor)) +
